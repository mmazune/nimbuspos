...
exports.seedComprehensive = seedComprehensive;
...
const seedPostingMappings_1 = require("./seedPostingMappings");
const seedPosReceipts_1 = require("./seedPosReceipts");
const seedInventoryGaps_1 = require("./seedInventoryGaps"); // M76: Depletions + COGS
const seedPrepItems_1 = require("./seedPrepItems"); // M80: Prep Items
const seedReportData_1 = require("./seedReportData"); // Report data: movements, waste, KDS, costs
...
async function seedReservations(prisma) {
    ...
    const createReservation = (id, orgId, branchId, tableId, name, phone, partySize, baseDate, startTime, endTime, status) => ({
        id,
        orgId,
        branchId,
        tableId,
    ...
    const reservationData = [
        ...
        createReservation('00000000-0000-4000-8000-000000004001', constants_1.ORG_TAPAS_ID, constants_1.BRANCH_TAPAS_MAIN_ID, TABLE_IDS.TAPAS[0], 'John Smith', '+256700111222', 2, (0, constants_1.getSeedDate)(-3), // 3 days ago relative to anchor
        '19:00', '21:00', 'SEATED'),
        createReservation('00000000-0000-4000-8000-000000004002', constants_1.ORG_TAPAS_ID, constants_1.BRANCH_TAPAS_MAIN_ID, TABLE_IDS.TAPAS[2], 'Mary Johnson', '+256700222333', 4, (0, constants_1.getSeedDate)(-2), // 2 days ago relative to anchor
        '18:30', '20:30', 'SEATED'),
        createReservation('00000000-0000-4000-8000-000000004003', constants_1.ORG_TAPAS_ID, constants_1.BRANCH_TAPAS_MAIN_ID, TABLE_IDS.TAPAS[4], 'David Williams', '+256700333444', 6, (0, constants_1.getSeedDate)(-1), // Yesterday relative to anchor
        '20:00', '22:00', 'CANCELLED'),
        // Tapas - Today's reservations
        createReservation('00000000-0000-4000-8000-000000004004', constants_1.ORG_TAPAS_ID, constants_1.BRANCH_TAPAS_MAIN_ID, TABLE_IDS.TAPAS[1], 'Sarah Brown', '+256700444555', 2, anchor, // Today (anchor date)
        '12:00', '14:00', 'CONFIRMED'),
        createReservation('00000000-0000-4000-8000-000000004005', constants_1.ORG_TAPAS_ID, constants_1.BRANCH_TAPAS_MAIN_ID, TABLE_IDS.TAPAS[3], 'Michael Davis', '+256700555666', 4, anchor, // Today (anchor date)
        '19:00', '21:00', 'CONFIRMED'),
        // Tapas - Future reservations
        createReservation('00000000-0000-4000-8000-000000004006', constants_1.ORG_TAPAS_ID, constants_1.BRANCH_TAPAS_MAIN_ID, TABLE_IDS.TAPAS[5], 'Emily Wilson', '+256700666777', 8, (0, constants_1.getSeedDate)(2), // 2 days from anchor
        '18:00', '21:00', 'HELD'),
        createReservation('00000000-0000-4000-8000-000000004007', constants_1.ORG_TAPAS_ID, constants_1.BRANCH_TAPAS_MAIN_ID, TABLE_IDS.TAPAS[6], 'Robert Taylor', '+256700777888', 2, (0, constants_1.getSeedDate)(5), // 5 days from anchor
        '20:00', '22:00', 'CONFIRMED'),
        // Cafesserie Village Mall reservations
        createReservation('00000000-0000-4000-8000-000000004101', constants_1.ORG_CAFESSERIE_ID, constants_1.BRANCH_CAFE_VILLAGE_MALL_ID, TABLE_IDS.CAFESSERIE_VILLAGE[0], 'Alice Thompson', '+256700888999', 2, anchor, // Today (anchor date)
        '10:00', '11:30', 'CONFIRMED'),
        createReservation('00000000-0000-4000-8000-000000004102', constants_1.ORG_CAFESSERIE_ID, constants_1.BRANCH_CAFE_VILLAGE_MALL_ID, TABLE_IDS.CAFESSERIE_VILLAGE[2], 'James Anderson', '+256700999000', 4, (0, constants_1.getSeedDate)(1), // Tomorrow relative to anchor
        '15:00', '17:00', 'HELD'),
    ];
 ...
 */
async function seedProcurement(prisma) {
    ...
    const anchor = constants_1.SEED_DATE_ANCHOR;
    const oneWeekAgo = (0, constants_1.getSeedDate)(-7);
    const twoWeeksAgo = (0, constants_1.getSeedDate)(-14);
    ...
    const tapasOwner = await prisma.user.findFirst({
        where: { orgId: constants_1.ORG_TAPAS_ID, roleLevel: 'L5' },
    });
    const cafeOwner = await prisma.user.findFirst({
        where: { orgId: constants_1.ORG_CAFESSERIE_ID, roleLevel: 'L5' },
    });
    ...
    for (const po of [...tapasPOs, ...cafePOs]) {
        const ownerId = po.orgId === constants_1.ORG_TAPAS_ID ? tapasOwner.id : cafeOwner.id;
    ...
    const poItemId = (poIdx, itemIdx) => `00000000-0000-4000-8000-0000000060${String(poIdx).padStart(2, '0')}${itemIdx}`;
    // Get or create default UOM for both orgs
    const tapasUom = await prisma.unitOfMeasure.upsert({
        where: { orgId_code: { orgId: constants_1.ORG_TAPAS_ID, code: 'pcs' } },
        update: {},
        create: { orgId: constants_1.ORG_TAPAS_ID, code: 'pcs', name: 'Pieces', symbol: 'pcs' },
    });
    const cafeUom = await prisma.unitOfMeasure.upsert({
        where: { orgId_code: { orgId: constants_1.ORG_CAFESSERIE_ID, code: 'pcs' } },
        update: {},
        create: { orgId: constants_1.ORG_CAFESSERIE_ID, code: 'pcs', name: 'Pieces', symbol: 'pcs' },
    });
    ...
    const tapasGRs = [
        {
            ...
            status: 'DRAFT', // M73: Add required status field
    ...
    const tapasLoc = await prisma.inventoryLocation.findFirst({
        where: { branchId: constants_1.BRANCH_TAPAS_MAIN_ID, name: 'Main Storage' },
    });
    const cafeLoc = await prisma.inventoryLocation.findFirst({
        where: { branchId: constants_1.BRANCH_CAFE_VILLAGE_MALL_ID, name: 'Main Storage' },
    });
    if (!tapasLoc || !cafeLoc) {
    ...
    const grLineId = (grIdx, itemIdx) => `00000000-0000-4000-8000-0000000070${String(grIdx).padStart(2, '0')}${itemIdx}`;
...
async function seedServiceProviders(prisma) {
    ...
    const contracts = [
        // Tapas contracts
        { id: '00000000-0000-4000-8000-00000000c001', providerId: SERVICE_PROVIDER_IDS.TAPAS[0], branchId: constants_1.BRANCH_TAPAS_MAIN_ID, frequency: 'MONTHLY', amount: 8500000, dueDay: 1, startDate: (0, constants_1.getSeedDate)(-365), notes: 'Monthly rent - Kololo premises' },
        { id: '00000000-0000-4000-8000-00000000c002', providerId: SERVICE_PROVIDER_IDS.TAPAS[1], branchId: constants_1.BRANCH_TAPAS_MAIN_ID, frequency: 'MONTHLY', amount: 450000, dueDay: 15, startDate: (0, constants_1.getSeedDate)(-365), notes: 'MTN 100Mbps fiber' },
        { id: '00000000-0000-4000-8000-00000000c003', providerId: SERVICE_PROVIDER_IDS.TAPAS[2], branchId: constants_1.BRANCH_TAPAS_MAIN_ID, frequency: 'MONTHLY', amount: 1200000, dueDay: 20, startDate: (0, constants_1.getSeedDate)(-365), notes: 'Electricity pre-paid top-up' },
        { id: '00000000-0000-4000-8000-00000000c004', providerId: SERVICE_PROVIDER_IDS.TAPAS[3], branchId: constants_1.BRANCH_TAPAS_MAIN_ID, frequency: 'WEEKLY', amount: 350000, dueDay: 1, startDate: (0, constants_1.getSeedDate)(-180), notes: 'Weekly deep clean - Monday' },
        { id: '00000000-0000-4000-8000-00000000c005', providerId: SERVICE_PROVIDER_IDS.TAPAS[4], branchId: constants_1.BRANCH_TAPAS_MAIN_ID, frequency: 'MONTHLY', amount: 2800000, dueDay: 5, startDate: (0, constants_1.getSeedDate)(-365), notes: '24/7 security guards (3 shifts)' },
        // Cafesserie contracts
        { id: '00000000-0000-4000-8000-00000000c101', providerId: SERVICE_PROVIDER_IDS.CAFESSERIE[0], branchId: constants_1.BRANCH_CAFE_VILLAGE_MALL_ID, frequency: 'MONTHLY', amount: 12000000, dueDay: 1, startDate: (0, constants_1.getSeedDate)(-365), notes: 'Mall rent + CAM fees' },
        { id: '00000000-0000-4000-8000-00000000c102', providerId: SERVICE_PROVIDER_IDS.CAFESSERIE[1], branchId: constants_1.BRANCH_CAFE_VILLAGE_MALL_ID, frequency: 'MONTHLY', amount: 350000, dueDay: 10, startDate: (0, constants_1.getSeedDate)(-180), notes: 'Airtel 50Mbps dedicated line' },
        { id: '00000000-0000-4000-8000-00000000c103', providerId: SERVICE_PROVIDER_IDS.CAFESSERIE[2], branchId: constants_1.BRANCH_CAFE_VILLAGE_MALL_ID, frequency: 'MONTHLY', amount: 180000, dueDay: 25, startDate: (0, constants_1.getSeedDate)(-365), notes: 'Water utility bill' },
    ];
    ...
    for (const c of contracts) {
        const orgId = c.providerId.startsWith('00000000-0000-4000-8000-000000008001') ||
            ...
            ? constants_1.ORG_TAPAS_ID : constants_1.ORG_CAFESSERIE_ID;
        if (c.frequency === 'MONTHLY') {
            ...
            if (daysUntilDue < 0) {
                ...
                severity = 'DUE_SOON';
                status = 'PAID'; // Future - already paid
            }
            reminders.push({
 ...
 */
async function seedOrdersForBranch(prisma, config) {
    const { branchId, orgId, tableIds, orderIdPrefix, daysBack = 30, ordersPerWeekday = 8, ordersPerWeekend = 12, } = config;
    ...
    const isTapas = orgId === constants_1.ORG_TAPAS_ID;
    ...
    for (const item of menuItems) {
        ...
        if (isTapas) {
            ...
            if (catName.includes('spirit') || catName.includes('vodka') || catName.includes('gin') || catName.includes('whiskey') || catName.includes('rum') || catName.includes('tequila') || catName.includes('brandy') || catName.includes('cream'))
                w = 6;
            else if (catName.includes('cocktail'))
                w = 5;
            else if (catName.includes('beer') || catName.includes('cider'))
                w = 4;
            else if (catName.includes('wine'))
                w = 3;
            else if (catName.includes('mocktail') || catName.includes('soft') || catName.includes('juice'))
                w = 2;
            else if (catName.includes('grill') || catName.includes('starter') || catName.includes('flat bread'))
                w = 2;
            else
                w = 1; // breakfast, desserts, etc.
        }
        else {
            // Café: coffee, specialty, tea, pastries dominate
            if (catName.includes('coffee') || catName.includes('specialty'))
                w = 6;
            else if (catName.includes('tea'))
                w = 4;
            else if (catName.includes('pastry') || catName.includes('pastries') || catName.includes('baked'))
                w = 4;
            else if (catName.includes('sandwich') || catName.includes('wrap'))
                w = 3;
            else if (catName.includes('breakfast'))
                w = 3;
            else if (catName.includes('smoothie') || catName.includes('juice') || catName.includes('cold drink'))
                w = 2;
            else
                w = 1;
        }
    ...
    const serviceStaff = allUsers.filter((u) => serviceRoles.includes(u.jobRole));
    // Fallback: if no service staff found, use all non-backoffice users
    const users = serviceStaff.length >= 2
        ? serviceStaff
        : allUsers.filter((u) => !['OWNER', 'ACCOUNTANT'].includes(u.jobRole)).slice(0, 5);
    if (users.length === 0) {
    ...
    for (const hw of hourWeights) {
        for (let i = 0; i < hw.weight; i++)
            hourPool.push(hw.hour);
    }
    ...
    for (let daysAgo = daysBack; daysAgo >= 0; daysAgo--) {
        ...
        const sampleDate = (0, constants_1.getSeedDate)(-daysAgo);
        ...
        for (let orderNum = 0; orderNum < ordersToday; orderNum++) {
            ...
            const orderDate = (0, constants_1.getSeedDate)(-daysAgo, hour);
 ...
 */
async function seedOpenOrders(prisma, config) {
    const { branchId, orgId, tableIds, orderIdPrefix, ordersCount = 12, } = config;
    ...
    const svcStaff = allUsers.filter((u) => svcRoles.includes(u.jobRole));
 ...
 */
async function seedCompletedOrders(prisma) {
 ...
 */
async function seedLiveOrders(prisma) {
 ...
 */
async function seedJournalEntries(prisma) {
    ...
    const orgs = [
        {
            ...
            prefix: '6',
            branches: [
                { id: constants_1.BRANCH_TAPAS_MAIN_ID, name: 'Main', multiplier: 1.0 }
            ]
        },
        {
            ...
            prefix: '7',
            branches: [
                { id: constants_1.BRANCH_CAFE_VILLAGE_MALL_ID, name: 'Village Mall', multiplier: 1.2 },
                { id: constants_1.BRANCH_CAFE_ACACIA_MALL_ID, name: 'Acacia Mall', multiplier: 1.0 },
                { id: constants_1.BRANCH_CAFE_ARENA_MALL_ID, name: 'Arena Mall', multiplier: 0.8 },
                { id: constants_1.BRANCH_CAFE_MOMBASA_ID, name: 'Mombasa', multiplier: 0.6 },
            ]
        },
    ];
    ...
    for (const org of orgs) {
        ...
        const serviceChargesAccount = accounts.find((a) => a.code === '4100');
        const cogsAccount = accounts.find((a) => a.code === '5000');
        const wastageAccount = accounts.find((a) => a.code === '5100');
        const payrollAccount = accounts.find((a) => a.code === '6000');
        const utilitiesAccount = accounts.find((a) => a.code === '6100');
        const rentAccount = accounts.find((a) => a.code === '6400');
        const suppliesAccount = accounts.find((a) => a.code === '6500');
        const marketingAccount = accounts.find((a) => a.code === '6600');
        ...
        async function upsertJE(id, data, lines) {
        ...
        for (const branch of org.branches) {
            ...
            for (let daysAgo = 30; daysAgo >= 0; daysAgo--) {
                ...
                if (cogsAccount && inventoryAccount) {
                    salesLines.push({ id: `${pfx}${pad}01-L3`, accountId: cogsAccount.id, debit: cogsAmount, credit: 0 }, { id: `${pfx}${pad}01-L4`, accountId: inventoryAccount.id, debit: 0, credit: cogsAmount });
                }
 ...
 */
async function seedBudgets(prisma) {
    console.log('\n💰 Seeding Budgets...');
    const orgs = [
        {
            id: constants_1.ORG_TAPAS_ID,
    ...
    let count = 0;
    const now = constants_1.SEED_DATE_ANCHOR;
    ...
    for (const user of cafeUsers) {
        ...
        await prisma.employee.upsert({
            ...
            create: {
                id: employeeId,
                orgId: constants_1.ORG_CAFESSERIE_ID,
                branchId: constants_1.BRANCH_CAFE_VILLAGE_MALL_ID, // Default to Village Mall branch
                userId: user.id,
 ...
 */
async function seedStaffAwards(prisma) {
    ...
    const admin = await prisma.user.findFirst({
        where: { orgId: constants_1.ORG_TAPAS_ID, roleLevel: 'L4' },
    });
    if (!admin) {
...
async function seedFeedback(prisma) {
    ...
    const getNpsScore = () => {
        ...
        if (rand < 0.85)
            return 7 + Math.floor(Math.random() * 2); // 7-8 (passives)
 ...
 */
async function seedEmployeeProfiles(prisma) {
 ...
 */
async function seedTimeEntries(prisma) {
 ...
 */
async function seedComprehensive(prisma) {
    ...
    try {
        ...
        await (0, seedLocations_1.seedInventoryLocations)(prisma); // M74: MOVED BEFORE procurement (GR lines need locations)
        await seedProcurement(prisma); // M33: POs + GRs
        await seedServiceProviders(prisma);
        await (0, seedPostingMappings_1.seedInventoryPostingMappings)(prisma); // NEW: GL posting mappings for inventory
        await seedInventory(prisma); // NEW: Inventory items + stock batches
        await (0, seedPrepItems_1.seedPrepItems)(prisma); // M80: Prep items
        await seedCompletedOrders(prisma);
        await seedLiveOrders(prisma); // NEW: OPEN orders for POS
        await (0, seedInventoryGaps_1.seedInventoryGaps)(prisma); // M76: Depletions + COGS breakdowns (depends on orders + inventory)
        await (0, seedReportData_1.seedReportData)(prisma); // Report data: movements, waste, KDS, menu costs
        await (0, seedPosReceipts_1.seedPosReceipts)(prisma); // M32: POS receipts for closed orders
        await (0, seedPosReceipts_1.seedCustomerReceipts)(prisma); // M32: Customer receipts for AR
...
//# sourceMappingURL=seedComprehensive.js.map