...
exports.seedInventoryGaps = seedInventoryGaps;
 ...
 */
function getDepletionId(orderId, index) {
 ...
 */
async function seedStockBatches(prisma, orgId, branchId, orgName) {
 ...
 */
async function seedDepletions(prisma, orgId, branchId, locationId, orgName) {
    ...
    for (let i = 0; i < Math.min(orders.length, 5); i++) {
        ...
        await prisma.orderInventoryDepletion.upsert({
            ...
            create: {
                ...
                branchId,
                locationId,
                status: db_1.DepletionStatus.SUCCESS,
                postedAt,
            },
            update: {
 ...
 */
async function seedInventoryGaps(prisma) {
    console.log('\n📦 M45: Seeding inventory gaps (levels + COGS)...');
    // Verify orgs exist
    const tapasOrg = await prisma.org.findUnique({ where: { id: constants_1.ORG_TAPAS_ID } });
    const cafeOrg = await prisma.org.findUnique({ where: { id: constants_1.ORG_CAFESSERIE_ID } });
    if (!tapasOrg) {
    ...
    await seedStockBatches(prisma, constants_1.ORG_TAPAS_ID, constants_1.BRANCH_TAPAS_MAIN_ID, 'Tapas');
    await seedStockBatches(prisma, constants_1.ORG_CAFESSERIE_ID, constants_1.BRANCH_CAFE_VILLAGE_MALL_ID, 'Cafesserie');
    // Seed depletions
    console.log('\n  📊 Seeding Depletions (for /inventory/cogs)...');
    await seedDepletions(prisma, constants_1.ORG_TAPAS_ID, constants_1.BRANCH_TAPAS_MAIN_ID, LOC_TAPAS_MAIN_ID, 'Tapas');
    await seedDepletions(prisma, constants_1.ORG_CAFESSERIE_ID, constants_1.BRANCH_CAFE_VILLAGE_MALL_ID, LOC_CAFE_VM_MAIN_ID, 'Cafesserie');
    console.log('\n  ✅ Inventory gap seeding complete');
}
//# sourceMappingURL=seedInventoryGaps.js.map