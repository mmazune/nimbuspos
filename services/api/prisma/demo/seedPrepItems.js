...
exports.seedPrepItems = seedPrepItems;
const client_1 = require("@prisma/client");
const constants_1 = require("./constants");
/**
 * Tapas prep items
 */
const TAPAS_PREP_ITEMS = [
    {
 ...
 */
async function seedPrepItems(prisma) {
    console.log('🔪 Seeding prep items...');
    // Seed Tapas prep items
    await seedOrgPrepItems(prisma, constants_1.ORG_TAPAS_ID, constants_1.BRANCH_TAPAS_MAIN_ID, TAPAS_PREP_ITEMS, 'Tapas');
    // Seed Cafesserie prep items
    await seedOrgPrepItems(prisma, constants_1.ORG_CAFESSERIE_ID, constants_1.BRANCH_CAFE_VILLAGE_MALL_ID, CAFESSERIE_PREP_ITEMS, 'Cafesserie');
    console.log('  ✅ Prep items seeded successfully');
    // M80 Phase 1: Link prep outputs to recipes (3+ per org)
    await linkPrepOutputsToRecipes(prisma, constants_1.ORG_TAPAS_ID, constants_1.BRANCH_TAPAS_MAIN_ID, 'Tapas');
    await linkPrepOutputsToRecipes(prisma, constants_1.ORG_CAFESSERIE_ID, constants_1.BRANCH_CAFE_VILLAGE_MALL_ID, 'Cafesserie');
}
/**
 * Seeds prep items for a specific org
 */
async function seedOrgPrepItems(prisma, orgId, branchId, prepItems, orgName) {
    ...
    const uomMap = new Map(uoms.map((u) => [u.code, u.id]));
    let created = 0;
    let skipped = 0;
    for (const itemData of prepItems) {
        ...
        for (const line of itemData.lines) {
            ...
            processedLines.push({
                ...
                qty: new client_1.Prisma.Decimal(line.qty),
                uomId: uomId, // Type-safe because we checked for null above
                notes: line.notes,
            });
        }
        ...
        await prisma.prepItem.create({
            data: {
                ...
                branchId,
                name: itemData.name,
                yieldQty: new client_1.Prisma.Decimal(itemData.yieldQty),
                yieldUomId,
 ...
 */
async function linkPrepOutputsToRecipes(prisma, orgId, _branchId, orgName) {
    ...
    for (let i = 0; i < Math.min(prepItems.length, recipesToUpdate.length); i++) {
        ...
        await prisma.recipeLine.create({
            data: {
                recipeId: recipe.id,
                inventoryItemId: prepItem.outputInventoryItemId,
                qtyInput: new client_1.Prisma.Decimal(qtyInput),
                inputUomId: uomId,
                qtyBase: new client_1.Prisma.Decimal(qtyInput),
...
//# sourceMappingURL=seedPrepItems.js.map