  ...
  const orgs = [
    {
      id: ORG_TAPAS_ID,
      name: 'Tapas',
      branches: [{ id: BRANCH_TAPAS_MAIN_ID, name: 'Main Branch' }],
    },
    {
  ...
  for (const org of orgs) {
    ...
    for (const branch of org.branches) {
      ...
      for (let daysAgo = 90; daysAgo >= 0; daysAgo--) {
        const baseDate = getSeedDate(-daysAgo);
        ...
        for (let v = 0; v < voidCount; v++) {
          ...
          const voidAmount = org.id === ORG_TAPAS_ID
        ...
        for (let r = 0; r < refundCount; r++) {
          ...
          const refundAmount = org.id === ORG_TAPAS_ID
...
// ============================================================
async function seedAnomalyEvents(prisma: PrismaClient): Promise<void> {
  console.log('\n⚠️  Seeding Anomaly Events (alerts)...');

  const orgs = [
    {
      id: ORG_TAPAS_ID,
      branches: [{ id: BRANCH_TAPAS_MAIN_ID }],
    },
    {
      id: ORG_CAFESSERIE_ID,
      branches: [
...
// ============================================================
async function seedFranchiseRankings(prisma: PrismaClient): Promise<void> {
  console.log('\n🏆 Seeding Franchise Rankings...');

  const branches = [
    { id: BRANCH_CAFE_VILLAGE_MALL_ID, name: 'Village Mall', revMultiplier: 1.3, wasteMultiplier: 0.7 },
    { id: BRANCH_CAFE_ACACIA_MALL_ID, name: 'Acacia Mall', revMultiplier: 1.1, wasteMultiplier: 0.9 },
    { id: BRANCH_CAFE_ARENA_MALL_ID, name: 'Arena Mall', revMultiplier: 0.85, wasteMultiplier: 1.2 },
    { id: BRANCH_CAFE_MOMBASA_ID, name: 'Mombasa', revMultiplier: 0.65, wasteMultiplier: 1.5 },
  ];
    ...
    for (let i = 0; i < branchScores.length; i++) {
      ...
      try {
        await prisma.franchiseRank.upsert({
          where: { orgId_period_branchId: { orgId: ORG_CAFESSERIE_ID, period, branchId: bs.branchId } },
          update: {
            score: bs.score,
            rank: i + 1,
            meta: bs.meta,
...
// ============================================================
async function seedFranchiseBudgets(prisma: PrismaClient): Promise<void> {
  console.log('\n💹 Seeding Franchise Budgets...');

  const branches = [
    { id: BRANCH_CAFE_VILLAGE_MALL_ID, budgetMultiplier: 1.3 },
    { id: BRANCH_CAFE_ACACIA_MALL_ID, budgetMultiplier: 1.1 },
    { id: BRANCH_CAFE_ARENA_MALL_ID, budgetMultiplier: 0.85 },
    { id: BRANCH_CAFE_MOMBASA_ID, budgetMultiplier: 0.65 },
  ];
...
// ============================================================
async function seedExpandedReservations(prisma: PrismaClient): Promise<void> {
  ...
  const statuses: Array<'CONFIRMED' | 'HELD' | 'CANCELLED'> = ['CONFIRMED', 'CONFIRMED', 'HELD', 'CANCELLED', 'CONFIRMED', 'CONFIRMED'];

  let count = 0;

  const branchConfigs = [
    { orgId: ORG_TAPAS_ID, branchId: BRANCH_TAPAS_MAIN_ID, tables: TABLE_IDS.TAPAS, prefix: 'T' },
    { orgId: ORG_CAFESSERIE_ID, branchId: BRANCH_CAFE_VILLAGE_MALL_ID, tables: TABLE_IDS.CAFESSERIE_VILLAGE, prefix: 'CV' },
    { orgId: ORG_CAFESSERIE_ID, branchId: BRANCH_CAFE_ACACIA_MALL_ID, tables: TABLE_IDS.CAFESSERIE_ACACIA, prefix: 'CA' },
    { orgId: ORG_CAFESSERIE_ID, branchId: BRANCH_CAFE_ARENA_MALL_ID, tables: TABLE_IDS.CAFESSERIE_ARENA, prefix: 'CR' },
    { orgId: ORG_CAFESSERIE_ID, branchId: BRANCH_CAFE_MOMBASA_ID, tables: TABLE_IDS.CAFESSERIE_MOMBASA, prefix: 'CM' },
  ];
  ...
  for (const config of branchConfigs) {
    ...
    for (let i = 0; i < 60; i++) {
      ...
      const reserveDate = getSeedDate(daysOffset);
...
// ============================================================
async function flagOrdersWithAnomalies(prisma: PrismaClient): Promise<void> {
  console.log('\n🚩 Flagging orders with anomaly flags...');

  // For each org, get recent orders and flag ~20% with NO_DRINKS
  const orgs = [ORG_TAPAS_ID, ORG_CAFESSERIE_ID];
  let flagged = 0;
...
// ============================================================
async function seedDiscounts(prisma: PrismaClient): Promise<void> {
  console.log('\n💸 Seeding Discounts...');
...
// ============================================================
async function seedExpandedJournalEntries(prisma: PrismaClient): Promise<void> {
  ...
  const orgs = [
    {
      ...
      branches: [
        ...
        { id: BRANCH_TAPAS_MAIN_ID, name: 'Main Branch', dailyRevBase: 20000000, dailyRevVar: 5000000 },
      ],
    },
    {
      id: ORG_CAFESSERIE_ID,
      ...
      branches: [
        ...
        { id: BRANCH_CAFE_VILLAGE_MALL_ID, name: 'Village Mall', dailyRevBase: 13000000, dailyRevVar: 3000000 },
        { id: BRANCH_CAFE_ACACIA_MALL_ID, name: 'Acacia Mall', dailyRevBase: 11000000, dailyRevVar: 2500000 },
        { id: BRANCH_CAFE_ARENA_MALL_ID, name: 'Arena Mall', dailyRevBase: 9000000, dailyRevVar: 2000000 },
        { id: BRANCH_CAFE_MOMBASA_ID, name: 'Mombasa', dailyRevBase: 7500000, dailyRevVar: 2000000 },
      ],
    },
  ];
    ...
    for (const branch of org.branches) {
      ...
      for (let daysAgo = 90; daysAgo >= 0; daysAgo--) {
        ...
        try {
          ...
          if (cogsAcc && invAcc) {
            lines.push(
              { id: `${entryId}-L3`, entryId, accountId: cogsAcc.id, debit: cogsAmount, credit: 0 },
              { id: `${entryId}-L4`, entryId, accountId: invAcc.id, debit: 0, credit: cogsAmount },
            );
          }
...
// ============================================================
async function seedExpandedFeedback(prisma: PrismaClient): Promise<void> {
  console.log('\n📝 Seeding Expanded Feedback (all branches)...');

  const branches = [
    { orgId: ORG_TAPAS_ID, branchId: BRANCH_TAPAS_MAIN_ID, prefix: 'T' },
    { orgId: ORG_CAFESSERIE_ID, branchId: BRANCH_CAFE_VILLAGE_MALL_ID, prefix: 'CV' },
    { orgId: ORG_CAFESSERIE_ID, branchId: BRANCH_CAFE_ACACIA_MALL_ID, prefix: 'CA' },
    { orgId: ORG_CAFESSERIE_ID, branchId: BRANCH_CAFE_ARENA_MALL_ID, prefix: 'CR' },
    { orgId: ORG_CAFESSERIE_ID, branchId: BRANCH_CAFE_MOMBASA_ID, prefix: 'CM' },
  ];
...
// ============================================================
async function seedExpandedTimeEntries(prisma: PrismaClient): Promise<void> {
  console.log('\n⏰ Seeding Expanded Time Entries (all branches, 30 days)...');

  const branches = [
    { orgId: ORG_CAFESSERIE_ID, branchId: BRANCH_CAFE_VILLAGE_MALL_ID },
    { orgId: ORG_CAFESSERIE_ID, branchId: BRANCH_CAFE_ACACIA_MALL_ID },
    { orgId: ORG_CAFESSERIE_ID, branchId: BRANCH_CAFE_ARENA_MALL_ID },
    { orgId: ORG_CAFESSERIE_ID, branchId: BRANCH_CAFE_MOMBASA_ID },
  ];
  ...
  for (const config of branches) {
    ...
    for (let daysAgo = 30; daysAgo >= 0; daysAgo--) {
      ...
      if (workDate.getDay() === 0) continue; // Skip Sundays

      for (const emp of employees) {
        if (rng() > 0.85) continue; // 15% absence rate
...
// MAIN EXPORT
// ============================================================
export async function seedRealisticExpansion(prisma: PrismaClient): Promise<void> {
  console.log('\n🚀 ═══════════════════════════════════════════════════');
