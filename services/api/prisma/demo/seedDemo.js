...
exports.printDemoCredentials = printDemoCredentials;
const argon2 = __importStar(require("argon2"));
const constants_1 = require("./constants");
const seedWorkforce_1 = require("./seedWorkforce");
 ...
 */
async function cleanupOldDemoData(prisma) {
    console.log('  🧹 Cleaning up old demo data...');
    const demoSlugs = [constants_1.TAPAS_ORG.slug, constants_1.CAFESSERIE_ORG.slug];
    const demoOrgIds = [constants_1.TAPAS_ORG.id, constants_1.CAFESSERIE_ORG.id];
 ...
 */
async function seedOrg(prisma, orgDef, branches, users) {
    console.log(`\n  🏢 Seeding ${orgDef.name}...`);
    ...
    const demoMetadata = {
        demoFreezeDate: constants_1.SEED_DATE_ANCHOR.toISOString(),
    };
    ...
    const passwordHash = await hashPassword(constants_1.DEMO_PASSWORD);
    // Create users
    for (const userDef of users) {
        ...
        const user = await prisma.user.upsert({
            ...
            update: {
                ...
                roleLevel: userDef.roleLevel,
                jobRole: userDef.jobRole, // M8.1: Populate jobRole for role-specific UX
                orgId: org.id,
 ...
 */
async function seedBadges(prisma, orgId, users) {
 ...
 */
async function seedChartOfAccounts(prisma, orgId) {
    ...
    if (cashAccount && bankAccount) {
        ...
        for (const m of mappings) {
            await prisma.paymentMethodMapping.upsert({
                where: { orgId_method: { orgId, method: m.method } },
                update: {},
                create: {
                    orgId,
                    method: m.method,
 ...
 */
async function seedFiscalPeriods(prisma, orgId) {
 ...
 */
async function seedVendorsAndBills(prisma, orgId) {
    ...
    for (const bill of billData) {
        const billDate = (0, constants_1.getSeedDate)(bill.days);
 ...
 */
async function seedCustomersAndInvoices(prisma, orgId) {
    ...
    for (const inv of invoiceData) {
        const invoiceDate = (0, constants_1.getSeedDate)(inv.days);
...
async function seedDemo(prisma) {
    ...
    await seedOrg(prisma, constants_1.TAPAS_ORG, constants_1.TAPAS_BRANCHES, constants_1.TAPAS_DEMO_USERS);
    await seedChartOfAccounts(prisma, constants_1.TAPAS_ORG.id);
    await seedFiscalPeriods(prisma, constants_1.TAPAS_ORG.id);
    await seedVendorsAndBills(prisma, constants_1.TAPAS_ORG.id);
    await seedCustomersAndInvoices(prisma, constants_1.TAPAS_ORG.id);
    // Seed Cafesserie
    await seedOrg(prisma, constants_1.CAFESSERIE_ORG, constants_1.CAFESSERIE_BRANCHES, constants_1.CAFESSERIE_DEMO_USERS);
    await seedChartOfAccounts(prisma, constants_1.CAFESSERIE_ORG.id);
    await seedFiscalPeriods(prisma, constants_1.CAFESSERIE_ORG.id);
    await seedVendorsAndBills(prisma, constants_1.CAFESSERIE_ORG.id);
    await seedCustomersAndInvoices(prisma, constants_1.CAFESSERIE_ORG.id);
    // Seed workforce data (M10.2)
    await (0, seedWorkforce_1.seedWorkforce)(prisma);
...
function printDemoCredentials() {
    ...
    constants_1.TAPAS_DEMO_USERS.forEach((user) => {
        const pin = user.pin ? ` (PIN: ${user.pin})` : '';
        console.log(`   ${user.email.padEnd(35)} / ${constants_1.DEMO_PASSWORD}${pin}`);
    });
    console.log('\n📍 Cafesserie:');
    constants_1.CAFESSERIE_DEMO_USERS.forEach((user) => {
        const pin = user.pin ? ` (PIN: ${user.pin})` : '';
        console.log(`   ${user.email.padEnd(35)} / ${constants_1.DEMO_PASSWORD}${pin}`);
    });
    console.log('\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
