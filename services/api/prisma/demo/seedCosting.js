...
exports.seedCosting = seedCosting;
const client_1 = require("@prisma/client");
const constants_1 = require("./constants");
/**
 * Seeds costing data for demo orgs
 */
async function seedCosting(prisma) {
    console.log('\n💰 Seeding Demo Costing Data (M35/M38)...');
    try {
        // Seed Recipe v2 for Tapas
        await seedRecipesV2(prisma, constants_1.ORG_TAPAS_ID, constants_1.BRANCH_TAPAS_MAIN_ID, 'Tapas');
        // Seed Recipe v2 for Cafesserie
        await seedRecipesV2(prisma, constants_1.ORG_CAFESSERIE_ID, constants_1.BRANCH_CAFE_VILLAGE_MALL_ID, 'Cafesserie');
        // M38: Seed ledger entries FIRST (for on-hand qty)
        await seedLedgerEntries(prisma, constants_1.ORG_TAPAS_ID, constants_1.BRANCH_TAPAS_MAIN_ID, constants_1.LOC_TAPAS_MAIN_ID, 'Tapas');
        await seedLedgerEntries(prisma, constants_1.ORG_CAFESSERIE_ID, constants_1.BRANCH_CAFE_VILLAGE_MALL_ID, constants_1.LOC_CAFE_VM_MAIN_ID, 'Cafesserie');
        // Seed cost layers for Tapas
        await seedCostLayers(prisma, constants_1.ORG_TAPAS_ID, constants_1.BRANCH_TAPAS_MAIN_ID, 'Tapas');
        // Seed cost layers for Cafesserie
        await seedCostLayers(prisma, constants_1.ORG_CAFESSERIE_ID, constants_1.BRANCH_CAFE_VILLAGE_MALL_ID, 'Cafesserie');
 ...
 */
async function seedRecipesV2(prisma, orgId, branchId, orgName) {
        ...
        const existing = await prisma.recipe.findUnique({
            where: {
                orgId_targetType_targetId: {
                    orgId,
                    targetType: client_1.RecipeTargetType.MENU_ITEM,
                    targetId: menuItemId,
                },
            },
        });
 ...
 */
async function seedCostLayers(prisma, orgId, branchId, orgName) {
    ...
    const seedDate = (0, constants_1.getSeedDate)(-30); // 30 days ago relative to SEED_DATE_ANCHOR
    for (const item of inventoryItems) {
        ...
        const existing = await prisma.inventoryCostLayer.findFirst({
            where: {
                ...
                branchId,
                itemId: item.id,
                sourceType: client_1.CostSourceType.INITIAL_SEED,
            },
        });
        if (existing)
            continue;
        ...
        await prisma.inventoryCostLayer.create({
            data: {
                ...
                branchId,
                itemId: item.id,
                method: client_1.CostMethod.WAC,
 ...
 */
async function seedLedgerEntries(prisma, orgId, branchId, locationId, orgName) {
 ...
 */
function generateDeterministicCost(identifier) {
...
//# sourceMappingURL=seedCosting.js.map