...
async function main() {
    ...
    const draftEntries = await db_1.prisma.journalEntry.updateMany({
        where: {
            orgId: { in: ORG_IDS },
            status: 'DRAFT',
        },
    ...
    for (const orgId of ORG_IDS) {
        const accounts = await db_1.prisma.account.findMany({ where: { orgId } });
        ...
        if (payrollAccId && utilitiesAccId) {
            const payrollEntries = await db_1.prisma.journalEntry.findMany({
                where: { orgId, memo: { contains: 'Payroll' } },
                select: { id: true },
            });
            if (payrollEntries.length > 0) {
                const fixedPayroll = await db_1.prisma.journalLine.updateMany({
                    where: {
                        entryId: { in: payrollEntries.map((e) => e.id) },
        ...
        if (rentAccId && wasteExpAccId) {
            const rentEntries = await db_1.prisma.journalEntry.findMany({
                where: { orgId, memo: { contains: 'rent', mode: 'insensitive' } },
                select: { id: true },
            });
            if (rentEntries.length > 0) {
                const fixedRent = await db_1.prisma.journalLine.updateMany({
                    where: {
                        entryId: { in: rentEntries.map((e) => e.id) },
    ...
    for (const orgId of ORG_IDS) {
        ...
        const total = await db_1.prisma.journalEntry.count({ where: { orgId } });
        const posted = await db_1.prisma.journalEntry.count({ where: { orgId, status: 'POSTED' } });
        const draft = await db_1.prisma.journalEntry.count({ where: { orgId, status: 'DRAFT' } });
...
//# sourceMappingURL=fix-finance-data.js.map