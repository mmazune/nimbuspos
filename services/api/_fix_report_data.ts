...
async function getOrCreateWaiters(prisma: PrismaClient, orgId: string, branchId: string): Promise<string[]> {
  // First check for existing service staff
  const serviceRoles: any[] = ['WAITER', 'BARTENDER', 'CASHIER'];
...
async function fixOrders(prisma: PrismaClient, orgId: string, branchId: string, waiterIds: string[]) {
...
async function fixReorderLevels(prisma: PrismaClient, orgId: string, branchId: string) {
