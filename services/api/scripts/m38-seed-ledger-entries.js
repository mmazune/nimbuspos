...
async function main() {
    ...
    if (!tapasOrg) {
        console.log(`❌ Tapas org not found (${constants_1.ORG_TAPAS_ID}). Run full seed first.`);
        process.exit(1);
    }
    if (!cafeOrg) {
        console.log(`❌ Cafesserie org not found (${constants_1.ORG_CAFESSERIE_ID}). Run full seed first.`);
        process.exit(1);
    }
    // Seed ledger entries for both orgs
    await seedLedgerEntries(constants_1.ORG_TAPAS_ID, constants_1.BRANCH_TAPAS_MAIN_ID, constants_1.LOC_TAPAS_MAIN_ID, 'Tapas Loco');
    await seedLedgerEntries(constants_1.ORG_CAFESSERIE_ID, constants_1.BRANCH_CAFE_VILLAGE_MALL_ID, constants_1.LOC_CAFE_VM_MAIN_ID, 'Cafesserie VM');
...
//# sourceMappingURL=m38-seed-ledger-entries.js.map