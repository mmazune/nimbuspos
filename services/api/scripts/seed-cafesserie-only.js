...
async function main() {
    ...
    const taxCategory = await withRetry(() => prisma.taxCategory.upsert({
    ...
    for (const branchId of branchIds) {
        ...
        for (const catData of CAFESSERIE_CATEGORIES) {
            const category = await withRetry(() => prisma.category.upsert({
                where: {
        ...
        for (const item of CAFESSERIE_MENU_ITEMS) {
            ...
            await withRetry(() => prisma.menuItem.upsert({
                where: {
    ...
    for (const branchId of branchIds.slice(0, 2)) {
        ...
        for (const tableData of tablesToCreate) {
            await withRetry(() => prisma.table.upsert({
                where: {
...
//# sourceMappingURL=seed-cafesserie-only.js.map