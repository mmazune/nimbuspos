...
function getCriticalFlows(role) {
...
function getRolesToAudit() {
    ...
    let configs = M69_ROLES.map(r => {
        const contractEntry = roleContract.results.find((rc) => rc.org === r.org && rc.role === r.role);
...
test_1.test.describe('Critical Flows Attribution', () => {
    test_1.test.setTimeout(400000); // 6.5 min per role
    for (const roleConfig of roles) {
        (0, test_1.test)(`Critical Flows ${roleConfig.org}/${roleConfig.role}`, async ({ page }) => {
...
//# sourceMappingURL=critical-flows-attribution.spec.js.map