...
function analyzeResponse(data) {
    if (Array.isArray(data)) {
        const hasValue = data.some((item) => {
            ...
            return ((item.onHand && Number(item.onHand) > 0) ||
...
const allEvidence = [];
test_1.test.describe('M45 UI Visibility Proof', () => {
    // Set test timeout
    test_1.test.setTimeout(120000); // 2 minutes total per role
    for (const roleConfig of ROLES) {
        test_1.test.describe(`${roleConfig.org}/${roleConfig.role}`, () => {
            let token;
            ...
            for (const route of ROUTES) {
                (0, test_1.test)(`${route.name}: ${route.endpoint}`, async () => {
                    test_1.test.setTimeout(ROUTE_TIMEOUT);
                    if (!token) {
                        test_1.test.skip();
                        return;
                    }
                    ...
                    if (route.endpoint === '/inventory/levels' || route.endpoint === '/inventory/cogs') {
                        (0, test_1.expect)(analysis.rowCount).toBeGreaterThan(0);
                    }
                });
            }
        });
...
//# sourceMappingURL=m45-ui-visibility-proof.spec.js.map