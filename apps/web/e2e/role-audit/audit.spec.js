...
`;
    for (const route of result.routesVisited) {
        const statusIcon = route.status === 'success' ? '✅' :
...
// Test Suite
// =============================================================================
test_1.test.describe('Role Audit Harness', () => {
    // Each role audit can take up to 5 minutes
    test_1.test.setTimeout(300_000);
    test_1.test.beforeAll(() => {
        ensureOutputDir();
    });
    const rolesToAudit = getRolesToAudit();
    for (const config of rolesToAudit) {
        (0, test_1.test)(`Audit ${config.org}/${config.role}`, async ({ page }) => {
            // Per-test timeout - 3.5 minutes for full crawl + cleanup
            test_1.test.setTimeout(210_000);
            const result = (0, types_1.createEmptyAuditResult)(config);
            ...
            await (0, login_1.blockNonEssentialResources)(page);
            // M30: Login with storage state caching for speed + stability
            const loginResult = await (0, login_1.loginWithCache)(page, config);
            ...
            if (!loginResult.success) {
                ...
                result.summary = (0, types_1.calculateSummary)(result);
            ...
            try {
                visibilityResult = await (0, visibility_1.verifyLandingPage)(page, config);
                ...
                console.log((0, visibility_1.formatVisibilityResult)(visibilityResult));
            }
            catch (err) {
            ...
            const boundedConfig = (0, types_1.getBoundedConfig)();
            ...
            for (const route of routesToVisit) {
                ...
                const { visit, controls, endpoints, failures, screenshot, totalClicksAfter, redundantCount } = await (0, crawler_1.visitRoute)(page, route, screenshotDir, config.role, // M16: Pass role for expected-forbidden classification
                ...
                for (const ep of endpoints) {
                    const existing = result.endpoints.find((e) => e.method === ep.method && e.path === ep.path);
                    if (existing) {
            ...
            if (phase1Exhausted) {
                ...
                for (const route of skippedRoutes) {
                    ...
                    const { visit, endpoints, failures, screenshot } = await (0, crawler_1.visitRouteQuick)(page, route, screenshotDir, config.role);
                    visitedRoutes.add(route);
                    result.routesVisited.push(visit);
                    // Merge endpoints
                    for (const ep of endpoints) {
