...
test_1.test.describe('INV15 - Tapas Procurement → Inventory Movement', () => {
    let token;
    test_1.test.beforeAll(async () => {
        token = await login(USERS.tapas.email, USERS.tapas.password);
    });
    (0, test_1.test)('INV15-TAP-1: Receipts list > 0', async () => {
        const receipts = await apiGet(token, '/inventory/receipts');
        (0, test_1.expect)(Array.isArray(receipts)).toBe(true);
        (0, test_1.expect)(receipts.length).toBeGreaterThan(0);
        console.log(`✅ Tapas: ${receipts.length} goods receipts`);
    });
    (0, test_1.test)('INV15-TAP-2: Receipt detail has lines > 0 with itemId', async () => {
        const receipts = await apiGet(token, '/inventory/receipts');
        const firstReceipt = receipts[0];
        const receiptDetail = await apiGet(token, `/inventory/receipts/${firstReceipt.id}`);
        (0, test_1.expect)(receiptDetail.lines).toBeDefined();
        (0, test_1.expect)(Array.isArray(receiptDetail.lines)).toBe(true);
        (0, test_1.expect)(receiptDetail.lines.length).toBeGreaterThan(0);
        // Check line has itemId
        const line = receiptDetail.lines[0];
        (0, test_1.expect)(line.itemId).toBeDefined();
        (0, test_1.expect)(typeof line.itemId).toBe('string');
        console.log(`✅ Tapas receipt ${receiptDetail.receiptNumber}: ${receiptDetail.lines.length} lines with itemIds`);
    });
    (0, test_1.test)('INV15-TAP-3: On-hand/levels returns non-empty', async () => {
        ...
        (0, test_1.expect)(firstLevel.itemId).toBeDefined();
        (0, test_1.expect)(firstLevel.onHand).toBeDefined();
        console.log(`✅ Tapas: ${levels.length} inventory levels (on-hand)`);
    });
    (0, test_1.test)('INV15-TAP-4: At least 3/5 receipt lines appear in on-hand with qty > 0', async () => {
        // Get first receipt lines
        const receipts = await apiGet(token, '/inventory/receipts');
        const firstReceipt = receipts[0];
        const receiptDetail = await apiGet(token, `/inventory/receipts/${firstReceipt.id}`);
        const linesToTrace = receiptDetail.lines.slice(0, 5);
        const itemIds = linesToTrace.map((line) => line.itemId);
        // Get on-hand levels
        const levels = await apiGet(token, '/inventory/levels');
...
test_1.test.describe('INV15 - Cafesserie Procurement → Inventory Movement', () => {
    let token;
    test_1.test.beforeAll(async () => {
        token = await login(USERS.cafesserie.email, USERS.cafesserie.password);
    });
    (0, test_1.test)('INV15-CAF-1: Receipts list > 0', async () => {
        const receipts = await apiGet(token, '/inventory/receipts');
        (0, test_1.expect)(Array.isArray(receipts)).toBe(true);
        (0, test_1.expect)(receipts.length).toBeGreaterThan(0);
        console.log(`✅ Cafesserie: ${receipts.length} goods receipts`);
    });
    (0, test_1.test)('INV15-CAF-2: Receipt detail has lines > 0 with itemId', async () => {
        const receipts = await apiGet(token, '/inventory/receipts');
        const firstReceipt = receipts[0];
        const receiptDetail = await apiGet(token, `/inventory/receipts/${firstReceipt.id}`);
        (0, test_1.expect)(receiptDetail.lines).toBeDefined();
    ...
    (0, test_1.test)('INV15-CAF-3: On-hand/levels returns non-empty', async () => {
        const levels = await apiGet(token, '/inventory/levels');
        (0, test_1.expect)(Array.isArray(levels)).toBe(true);
        (0, test_1.expect)(levels.length).toBeGreaterThan(0);
        // Check structure
    ...
    (0, test_1.test)('INV15-CAF-4: At least 3/5 receipt lines appear in on-hand with qty > 0', async () => {
        // Get first receipt lines
        const receipts = await apiGet(token, '/inventory/receipts');
        const firstReceipt = receipts[0];
        const receiptDetail = await apiGet(token, `/inventory/receipts/${firstReceipt.id}`);
    ...
    (0, test_1.test)('INV15-CAF-5: Valuation total > 0 (if accessible)', async () => {
        try {
            const valuation = await apiGet(token, '/inventory/valuation');
            if (Array.isArray(valuation)) {
                (0, test_1.expect)(valuation.length).toBeGreaterThan(0);
...
//# sourceMappingURL=seed-invariants-v15.spec.js.map