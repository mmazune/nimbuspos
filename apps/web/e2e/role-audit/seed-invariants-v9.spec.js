...
// Test Suite
// =============================================================================
test_1.test.describe('Seed Invariants v9 - Costing Reconciliation', () => {
    ...
    for (const org of ORGS) {
        test_1.test.describe(org.name, () => {
            ...
            (0, test_1.test)('INV9-1: Valuation has lines and non-zero total', async () => {
            ...
            (0, test_1.test)('INV9-2: At least 10 items have non-zero WAC', async () => {
                const { status, data } = await fetchWithAuth('/inventory/valuation?includeZeroStock=true', ownerToken);
                const valData = data?.data;
                const lines = valData?.lines || [];
            ...
            (0, test_1.test)('INV9-3: At least 10 recipes have ingredient lines', async () => {
            ...
            (0, test_1.test)('INV9-4: At least 5 recipes have computable costs', async () => {
                ...
                const valLines = valRes.data?.data?.lines || [];
                ...
                for (const recipe of recipes.slice(0, 15)) {
                    ...
                    const recipeData = recipeDetail.data;
            ...
            (0, test_1.test)('INV9-5: COGS endpoint returns valid structure', async () => {
                const { status, data } = await fetchWithAuth('/inventory/cogs?fromDate=2025-01-01&toDate=2026-12-31', ownerToken);
                const cogsData = data?.data;
                ...
                (0, test_1.expect)(isValidStructure).toBe(true);
            });
            // INV9-6: At least 2 reporting endpoints return valid data
            (0, test_1.test)('INV9-6: Reporting endpoints return valid data', async () => {
                const endpoints = [
                ...
                for (const ep of endpoints) {
                    ...
                    const hasData = status === 200 && data !== null && Object.keys(data).length > 0;
                    if (hasData) {
                        validEndpoints++;
...
//# sourceMappingURL=seed-invariants-v9.spec.js.map