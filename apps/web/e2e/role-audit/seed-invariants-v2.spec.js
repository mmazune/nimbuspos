...
async function writeResults() {
    ...
    for (const r of results) {
        ...
        mdLines.push(`| ${r.id} | ${r.org} | ${r.role} | ${r.name} | ${status} | \`${r.endpoint}\` | ${r.durationMs}ms |`);
    }
...
// Test Suite
// =============================================================================
test_1.test.describe('Seed Invariants v2', () => {
    ...
    for (const org of ['tapas', 'cafesserie']) {
        (0, test_1.test)(`${org}/manager - M1: Dashboard KPIs accessible`, async () => {
            ...
            try {
                ...
                results.push(result);
                (0, test_1.expect)(isAccessible, `${org}/manager should access analytics`).toBe(true);
            }
            catch (err) {
                results.push({
                    id: `M1-${org}`,
    ...
    for (const org of ['tapas', 'cafesserie']) {
        (0, test_1.test)(`${org}/manager - M2: Staff list visible`, async () => {
            const start = Date.now();
            const key = `${org}/manager`;
            const endpoint = '/hr/employees';
            try {
                const { status, data } = await fetchWithAuth(endpoint, tokens[key]);
                const employees = Array.isArray(data) ? data : data?.items || data?.employees || [];
                ...
                results.push(result);
                (0, test_1.expect)(hasStaff, `${org}/manager should see staff`).toBe(true);
            }
            catch (err) {
                results.push({
                    id: `M2-${org}`,
    ...
    for (const org of ['tapas']) {
        (0, test_1.test)(`${org}/stock - S1: Inventory items accessible`, async () => {
            const start = Date.now();
            const key = `${org}/stock`;
            ...
            try {
                ...
                results.push(result);
                (0, test_1.expect)(hasItems, `${org}/stock should see inventory items`).toBe(true);
            }
            catch (err) {
                results.push({
                    id: `S1-${org}`,
    ...
    for (const org of ['tapas']) {
        (0, test_1.test)(`${org}/stock - S2: Stock levels accessible`, async () => {
            const start = Date.now();
            const key = `${org}/stock`;
            ...
            try {
                ...
                results.push(result);
                (0, test_1.expect)(hasLevels, `${org}/stock should access stock levels`).toBe(true);
            }
            catch (err) {
                results.push({
                    id: `S2-${org}`,
    ...
    for (const org of ['tapas', 'cafesserie']) {
        (0, test_1.test)(`${org}/procurement - P1: Suppliers list accessible`, async () => {
            const start = Date.now();
            ...
            try {
                ...
                results.push(result);
                (0, test_1.expect)(hasSuppliers, `${org}/procurement should see suppliers`).toBe(true);
            }
            catch (err) {
                results.push({
                    id: `P1-${org}`,
    ...
    for (const org of ['tapas', 'cafesserie']) {
        (0, test_1.test)(`${org}/procurement - P2: Purchase orders visible`, async () => {
            const start = Date.now();
            const key = `${org}/procurement`;
            ...
            try {
                ...
                results.push(result);
                (0, test_1.expect)(hasOrders, `${org}/procurement should see POs`).toBe(true);
            }
            catch (err) {
                results.push({
                    id: `P2-${org}`,
...
//# sourceMappingURL=seed-invariants-v2.spec.js.map