...
// Test Suite
// =============================================================================
test_1.test.describe('Seed Invariants v1', () => {
    // Store tokens for each org
    const tokens = { tapas: '', cafesserie: '' };
    ...
    // -------------------------------------------------------------------------
    for (const owner of OWNERS) {
        (0, test_1.test)(`${owner.org}/owner - INV1: POS menu items > 0`, async () => {
            ...
            try {
                ...
                results.push(result);
                (0, test_1.expect)(status, `${owner.org} should have accessible top-items`).toBe(200);
            }
            catch (err) {
                results.push({
                    id: `INV1-${owner.org}`,
    ...
    // -------------------------------------------------------------------------
    for (const owner of OWNERS) {
        (0, test_1.test)(`${owner.org}/owner - INV2: Orders exist (analytics)`, async () => {
            const start = Date.now();
            const endpoint = '/analytics/daily';
            try {
                const { status, data } = await fetchWithAuth(endpoint, tokens[owner.org]);
                // Analytics should return some data structure indicating orders exist
                const hasData = status === 200 && data && Object.keys(data).length > 0;
                const result = {
                    id: `INV2-${owner.org}`,
                    org: owner.org,
                    name: 'Orders Exist',
                ...
                results.push(result);
                (0, test_1.expect)(hasData, `${owner.org} should have analytics data`).toBe(true);
            }
            catch (err) {
                results.push({
                    id: `INV2-${owner.org}`,
    ...
    // -------------------------------------------------------------------------
    for (const owner of OWNERS) {
        (0, test_1.test)(`${owner.org}/owner - INV3: Inventory items > 0`, async () => {
            const start = Date.now();
            ...
            try {
                ...
                results.push(result);
                (0, test_1.expect)(count, `${owner.org} should have inventory items`).toBeGreaterThan(0);
            }
            catch (err) {
                results.push({
                    id: `INV3-${owner.org}`,
    ...
    // -------------------------------------------------------------------------
    for (const owner of OWNERS) {
        (0, test_1.test)(`${owner.org}/owner - INV4: Menu-Inventory linkage`, async () => {
            const start = Date.now();
            // Use inventory levels as proxy for menu-inventory linkage
            const endpoint = '/inventory/levels';
            try {
                const { status, data } = await fetchWithAuth(endpoint, tokens[owner.org]);
                const levels = Array.isArray(data) ? data : data?.levels || data?.items || [];
                ...
                results.push(result);
                (0, test_1.expect)(hasLinkage, `${owner.org} inventory levels accessible`).toBe(true);
            }
            catch (err) {
                results.push({
                    id: `INV4-${owner.org}`,
    ...
    // -------------------------------------------------------------------------
    for (const owner of OWNERS) {
        (0, test_1.test)(`${owner.org}/owner - INV5: Finance endpoints non-empty`, async () => {
            const start = Date.now();
            ...
            try {
                ...
                results.push(result);
                (0, test_1.expect)(status, `${owner.org} trial balance accessible`).toBe(200);
            }
            catch (err) {
                results.push({
                    id: `INV5-${owner.org}`,
...
async function writeResults() {
    ...
    // Write JSON
    fs.writeFileSync(path.join(OUTPUT_DIR, 'SEED_INVARIANTS.v1.json'), JSON.stringify(report, null, 2));
    // Write Markdown
...
function generateMarkdown(report) {
    ...
    for (const [org, invs] of Object.entries(byOrg)) {
        ...
        for (const inv of invs) {
            ...
            lines.push(`| ${inv.id} | ${inv.name} | ${statusIcon} ${inv.status} | ${inv.expected} | ${inv.actual} | ${inv.durationMs}ms |`);
        }
        lines.push('');
    }
    // Failures section
...
//# sourceMappingURL=seed-invariants.spec.js.map