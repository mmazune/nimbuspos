...
function getControlsForRole(registry, org, role) {
    ...
    return registry.controls
        .filter((c) => c.org === org && c.roleId === role)
        .map((c, index) => ({
        ...c,
...
const allResults = [];
test_1.test.describe('Attribution Audit', () => {
    test_1.test.setTimeout(600000); // M65: 10 min per role (increased from 200s for full attribution)
    for (const roleConfig of roles) {
        (0, test_1.test)(`Attribution ${roleConfig.org}/${roleConfig.role}`, async ({ page }) => {
            ...
            await page.evaluate(() => {
                window.__E2E_ACTION_TRACE__ = true;
            });
            ...
            const routes = await (0, crawler_1.discoverRoutes)(page, roleConfig.org, roleConfig.role);
                    ...
                    for (const ctrl of routeControls) {
                        ...
                        try {
                            ...
                            // PRIORITY 3: Parse locatorStrategy from registry
                                ...
                                if (ctrl.locatorStrategy.includes('getByRole')) {
                                    ...
                                    if (match) {
                                        ...
                                        locator = name ? page.getByRole(role, { name }) : page.getByRole(role);
                                    }
                                }
                            ...
                            await page.evaluate((aid) => {
                                window.__E2E_CURRENT_ACTION__ = aid;
                            }, actionId);
                            ...
                            await page.evaluate(() => {
                                window.__E2E_CURRENT_ACTION__ = null;
                            });
                            // Collect API calls made during this action
                            const apiCountAfter = apiCalls.length;
                            const actionCalls = apiCalls.slice(apiCountBefore).filter(c => c.actionId === actionId || c.timestamp > apiCountBefore);
            ...
            const uniqueEndpointSet = new Set();
            ...
            if (attributionList.length < expectedMinControls && roleControls.length >= expectedMinControls) {
                throw new Error(`M67 PERSISTENCE GUARDRAIL FAILED: Only ${attributionList.length} controls persisted, but ${roleControls.length} controls were available. ` +
            ...
            const controlsWithBlockedMutations = attributionList.filter(c => c.endpoints.some(ep => ep.status === 999)).length;
            const summary = {
...
// Output Generators
// =============================================================================
function generateAttributionMarkdown(result) {
    const lines = [
...
function generateAggregatedMap(results) {
        ...
        for (const ctrl of result.controls) {
            ...
            if (ctrl.attribution === 'HAS_ENDPOINTS') {
                ...
                for (const ep of ctrl.endpoints) {
                    ...
                    const exists = endpointsByControl[controlKey].some(e => e.method === ep.method && e.path === ep.path);
                    if (!exists) {
...
function generateAggregatedMarkdown(map) {
    const lines = [
...
//# sourceMappingURL=attribution-audit.spec.js.map