...
test_1.test.describe('M77: COGS Pipeline Idempotency', () => {
    (0, test_1.test)('M77-IDEMP-1: Tapas COGS is deterministic after seed reruns', async () => {
        ...
        (0, test_1.expect)(normalized1.lineCount, 'M76 baseline: 5 COGS lines').toBe(5);
        (0, test_1.expect)(normalized1.totalCogs, 'M76 baseline: 75000 total COGS').toBe(75000);
        ...
        (0, test_1.expect)(normalized2.lineCount, 'Line count must be identical').toBe(normalized1.lineCount);
        (0, test_1.expect)(normalized2.totalCogs, 'Total COGS must be identical').toBe(normalized1.totalCogs);
        // Deep comparison of line items (order-independent)
        (0, test_1.expect)(normalized2.lines, 'All line items must match exactly').toEqual(normalized1.lines);
    ...
    (0, test_1.test)('M77-IDEMP-2: Cafesserie COGS is deterministic after seed reruns', async () => {
        // First pass: Get baseline COGS
    ...
    (0, test_1.test)('M77-IDEMP-3: COGS stable ordering (sorted by itemId then qty)', async () => {
        ...
        for (const [itemId, qtys] of itemGroups) {
            ...
            (0, test_1.expect)(qtys, `ItemId ${itemId} quantities must be sorted`).toEqual(sortedQtys);
        }
        console.log('✅ COGS stable ordering verified for Tapas');
    });
});
//# sourceMappingURL=m77-idempotency.spec.js.map