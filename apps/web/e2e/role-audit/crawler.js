...
exports.loadRoleContractRoutes = loadRoleContractRoutes;
exports.discoverRoutes = discoverRoutes;
exports.discoverControls = discoverControls;
exports.createNetworkWatcher = createNetworkWatcher;
exports.updateTrigger = updateTrigger;
exports.visitRoute = visitRoute;
exports.visitRouteQuick = visitRouteQuick;
 ...
 */
async function safePageOperation(page, operation, fallback) {
    if (!isPageValid(page)) {
        return fallback;
    }
    try {
 ...
 */
function loadRoleContractRoutes(org, role) {
    try {
        const fs = require('fs');
        ...
        const roleEntry = contract.results?.find((r) => r.org === org && r.role === role);
        if (!roleEntry) {
...
async function discoverRoutes(page, org, role) {
...
const CONTROL_SELECTORS = [
    ...
    { selector: '[data-testid*="tab"]', type: 'tab' },
    // Filters and dropdowns
    { selector: '[data-testid*="filter"]', type: 'filter' },
    { selector: '[data-testid*="select"]', type: 'dropdown' },
    { selector: 'select', type: 'dropdown' },
    // Date pickers
    { selector: '[data-testid*="date"]', type: 'date-picker' },
    { selector: 'input[type="date"]', type: 'date-picker' },
    // Search
    { selector: '[data-testid*="search"]', type: 'search' },
    { selector: 'input[type="search"]', type: 'search' },
    { selector: 'input[placeholder*="search" i]', type: 'search' },
    // Pagination
    { selector: '[data-testid*="pagination"]', type: 'pagination' },
    { selector: '[aria-label*="page"]', type: 'pagination' },
    { selector: '[aria-label*="next"]', type: 'pagination' },
    { selector: '[aria-label*="previous"]', type: 'pagination' },
    // Toggle
    { selector: '[role="switch"]', type: 'toggle' },
    { selector: '[data-testid*="toggle"]', type: 'toggle' },
    // Menu triggers
    { selector: '[data-testid*="menu"]', type: 'menu' },
    { selector: '[aria-haspopup="menu"]', type: 'menu' },
    // Modal triggers (non-destructive)
    { selector: '[data-testid*="modal"]', type: 'modal-trigger' },
    { selector: '[data-testid*="dialog"]', type: 'modal-trigger' },
    // Icon buttons
    { selector: 'button svg', type: 'icon-button' },
    { selector: '[role="button"] svg', type: 'icon-button' },
    // Regular buttons (checked last)
    { selector: 'button:not([type="submit"])', type: 'button' },
 ...
 */
async function discoverControls(page, route, config) {
    const boundedConfig = config || (0, types_1.getBoundedConfig)();
    const maxControls = boundedConfig.maxControlsPerRoute;
    const controls = [];
    const seen = new Set();
    // Priority types for bounded mode sampling
    const highPriorityTypes = ['tab', 'filter', 'search', 'pagination', 'date-picker', 'dropdown'];
    for (const { selector, type } of CONTROL_SELECTORS) {
        ...
        try {
            ...
            for (let i = 0; i < Math.min(elements.length, maxPerSelector); i++) {
                ...
                // Determine if safe
                const safeToClick = !(0, types_1.isUnsafe)(label) &&
                    !(0, types_1.isUnsafe)(testId || '') &&
                    !(0, types_1.isUnsafeSelector)(controlSelector);
                controls.push({
 ...
 */
function createNetworkWatcher(page, route, role) {
    const capture = {
        requests: new Map(),
        failures: [],
    ...
    page.on('response', (response) => {
        ...
        if (status === 401) {
            ...
            if (role && (0, types_1.isExpectedForbidden)(role, path)) {
 ...
 */
function updateTrigger(capture, trigger) {
 ...
 */
function computeFingerprint(endpoints) {
 ...
 */
async function visitRoute(page, route, screenshotDir, role, config, seenFingerprints, totalClicksSoFar = 0) {
    const boundedConfig = config || (0, types_1.getBoundedConfig)();
    const fingerprints = seenFingerprints || new Set();
    ...
    try {
        ...
        await (0, login_1.waitForPageReady)(page);
...
async function visitRouteQuick(page, route, screenshotDir, role) {
    const startTime = Date.now();
    let screenshot;
    // Check if page is still valid before starting
    if (!isPageValid(page)) {
