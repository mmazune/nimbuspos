...
const allResults = [];
test_1.test.describe('Route-Load Attribution', () => {
    test_1.test.setTimeout(200000); // 3+ min per role
    for (const roleConfig of roles) {
        (0, test_1.test)(`Route-Load ${roleConfig.org}/${roleConfig.role}`, async ({ page }) => {
            const startTime = Date.now();
            const routeAttributions = [];
...
// Output Generators
// =============================================================================
function generateRouteLoadMarkdown(result) {
    const lines = [
    ...
    for (const result of results) {
        ...
        for (const route of result.routes) {
            ...
            for (const call of route.apiCalls) {
                ...
                const existingIdx = endpointsByRoute[route.route].findIndex(e => e.method === call.method && e.path === call.path);
                if (existingIdx >= 0) {
...
//# sourceMappingURL=route-load-attribution.spec.js.map