 ...
 */
function getExpectedSidebarLinks(roleId) {
...
// =============================================================================
const allResults = [];
test_1.test.describe('Role Landing + Sidebar Contract', () => {
    test_1.test.setTimeout(120000); // 2 min per test
    ...
    for (const config of types_1.ROLE_CONFIGS) {
        (0, test_1.test)(`${config.org}/${config.role}: login → landing → sidebar`, async ({ page, }) => {
            ...
            try {
                ...
                const loginResult = await (0, login_1.loginAsRole)(page, config);
                ...
                result.sidebarUnexpectedLinks = sidebarLinks.filter((link) => !expectedLinks.some((exp) => link === exp || link.startsWith(exp + '/')));
                // Check for missing links (in expected but not in sidebar)
                // Note: This is informational - some links might be in dropdowns
                result.sidebarMissingLinks = expectedLinks.filter((exp) => !sidebarLinks.some((link) => link === exp || exp.startsWith(link)));
                ...
                (0, test_1.expect)(result.landingMatch, `Landing mismatch: expected ${config.expectedLanding}, got ${result.landingActual}`).toBe(true);
...
//# sourceMappingURL=role-contract.spec.js.map