...
// Invariant Tests
// =============================================================================
test_1.test.describe('Seed Invariants v6 - Menu/Costing Realism', () => {
    ...
    for (const org of ORGS) {
        test_1.test.describe(`${org.name}`, () => {
            ...
            (0, test_1.test)('INV-V6-1: Recipes visible to Chef > 0', async () => {
            ...
            (0, test_1.test)('INV-V6-2: Valuation endpoint returns rows with cost layers', async () => {
            ...
            (0, test_1.test)('INV-V6-3: COGS endpoint returns valid breakdown structure', async () => {
            ...
            (0, test_1.test)('INV-V6-4: Recipe lines have non-zero cost (via inventory WAC)', async () => {
                // Fetch recipes with lines
                const { status, data } = await fetchWithAuth('/inventory/v2/recipes?includeLines=true&limit=10', ownerToken);
                const response = data;
                const recipes = Array.isArray(response?.recipes)
                    ? response.recipes
                    : Array.isArray(response?.data)
                ...
                for (const itemId of itemIdArray) {
                    const { status: itemStatus, data: itemData } = await fetchWithAuth(`/inventory/items/${itemId}`, ownerToken);
                    if (itemStatus === 200) {
                        const item = itemData;
            ...
            (0, test_1.test)('INV-V6-5: Low-stock ingredient exists in recipe linkage', async () => {
                ...
                const { data: recipeData } = await fetchWithAuth('/inventory/v2/recipes?includeLines=true&limit=50', ownerToken);
                ...
                recordResult({
                    ...
                    details: lowStockNames.length > 0
...
//# sourceMappingURL=seed-invariants-v6.spec.js.map