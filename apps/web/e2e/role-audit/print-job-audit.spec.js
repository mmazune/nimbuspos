...
function saveRoleResult(result) {
    ...
    fs.writeFileSync(path.join(dir, `${filename}.json`), JSON.stringify(result, null, 2));
 ...
 */
async function injectPrintInterceptor(page) {
    await page.addInitScript(() => {
        window.__print_calls = 0;
        const originalPrint = window.print;
        window.print = function () {
            window.__print_calls++;
            console.log('[INTERCEPTOR] window.print() called, count:', window.__print_calls);
 ...
 */
async function wasWindowPrintCalled(page) {
    try {
        const count = await page.evaluate(() => window.__print_calls || 0);
        return count > 0;
    }
    catch {
        return false;
 ...
 */
async function resetPrintCounter(page) {
    try {
        await page.evaluate(() => {
            window.__print_calls = 0;
        });
 ...
 */
async function getReceiptIds(page, org, limit) {
    try {
 ...
 */
async function testReceiptDetail(page, receiptId, org) {
 ...
 */
async function discoverPrintControls(page, route) {
    const controls = [];
 ...
 */
async function classifyControl(page, control, route) {
    const result = {
        route,
...
// Test Suite
// =============================================================================
test_1.test.describe('M51: UI-Only Print + Receipt Sampling Audit', () => {
    test_1.test.setTimeout(180000); // 3 minutes per test
    for (const roleConfig of ROLES_TO_AUDIT) {
        (0, test_1.test)(`${roleConfig.org}/${roleConfig.role}`, async ({ page }) => {
...
test_1.test.afterAll(async () => {
    ...
    const uiOnlyPrints = allResults.flatMap(r => r.controls.filter(c => c.classification === 'UI_ONLY_PRINT'));
    if (uiOnlyPrints.length > 0) {
        fs.writeFileSync(path.join(dir, 'UI_ONLY_PRINTS.v1.json'), JSON.stringify({
    ...
    fs.writeFileSync(path.join(dir, 'PRINT_EXPORT_CONTROLS.v3.json'), JSON.stringify(merged, null, 2));
    ...
    fs.writeFileSync(path.join(dir, 'PRINT_EXPORT_CONTROLS.v3.md'), mdLines.join('\n'));
    console.log(`\n[M51] v3 reports saved to ${dir}/`);
});
//# sourceMappingURL=print-job-audit.spec.js.map