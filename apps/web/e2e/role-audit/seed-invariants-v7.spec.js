...
// Invariant Tests
// =============================================================================
test_1.test.describe('Seed Invariants v7 - Valuation/COGS Non-Zero', () => {
    ...
    for (const org of ORGS) {
        test_1.test.describe(`${org.name}`, () => {
            ...
            (0, test_1.test)('INV-V7-1: Valuation endpoint returns > 0 rows', async () => {
                const { status, data } = await fetchWithAuth('/inventory/valuation?includeZeroStock=true', managerToken);
            ...
            (0, test_1.test)('INV-V7-2: Valuation total amount > 0', async () => {
                const { status, data } = await fetchWithAuth('/inventory/valuation?includeZeroStock=true', managerToken);
                const valuation = data;
            ...
            (0, test_1.test)('INV-V7-3: COGS endpoint returns structure with at least one numeric field > 0', async () => {
            ...
            (0, test_1.test)('INV-V7-4: At least 5 recipes have ingredients with non-zero WAC', async () => {
                // Fetch recipes with lines - check if ingredient items have WAC via valuation
                const { status, data } = await fetchWithAuth('/inventory/v2/recipes?includeLines=true&limit=100', ownerToken);
                const response = data;
                ...
                const { status: valStatus, data: valData } = await fetchWithAuth('/inventory/valuation?includeZeroStock=true', managerToken);
                const valuation = valData;
                // Build set of items with non-zero WAC
                const itemsWithWac = new Set();
                ...
                const recipesWithCostedIngredients = new Set();
                ...
                recordResult({
                    ...
                    expected: '>= 5',
                    details: recipesWithCost > 0
                        ? `examples: ${Array.from(recipesWithCostedIngredients).slice(0, 3).join(', ')}`
                        : 'No recipes have costed ingredients',
                });
                (0, test_1.expect)(status).toBe(200);
                (0, test_1.expect)(recipesWithCost).toBeGreaterThanOrEqual(5);
            });
            (0, test_1.test)('INV-V7-5: Low-stock ingredient linkage still holds (regression check)', async () => {
                // This is a regression check - ensure recipe-ingredient links still work
                // Fetch recipes with lines
                const { status, data } = await fetchWithAuth('/inventory/v2/recipes?includeLines=true&limit=50', ownerToken);
                const response = data;
                const recipes = Array.isArray(response?.recipes)
                    ? response.recipes
                    : Array.isArray(response?.data)
                ...
                (0, test_1.expect)(status).toBe(200);
                (0, test_1.expect)(recipesWithIngredients).toBeGreaterThan(0);
                (0, test_1.expect)(totalIngredientLines).toBeGreaterThan(0);
            });
        });
    }
...
//# sourceMappingURL=seed-invariants-v7.spec.js.map