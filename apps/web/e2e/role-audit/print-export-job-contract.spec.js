...
const SUBSET_ROLES = ALL_ROLES.filter((r) => (r.org === 'tapas' && (r.role === 'owner' || r.role === 'cashier')) ||
...
function saveReport(report) {
    ...
    fs.writeFileSync(path.join(OUTPUT_DIR, `${basename}.md`), md);
}
 ...
 */
async function getReceiptIds(page, org) {
    try {
...
async function testReceiptPage(page, receiptId) {
    ...
    if (buttonFound) {
        ...
        printCalls = await page.evaluate(() => window.__print_calls || 0);
    }
 ...
 */
async function classifyControl(page, route, locator) {
    ...
    await page.evaluate(() => {
        window.__print_calls = 0;
        window.__download_detected = null;
        window.__async_job_response = null;
    });
    // Listen for responses
    const responses = [];
    const responseHandler = (resp) => responses.push(resp);
    ...
    try {
        ...
        const downloadResp = responses.find((r) => {
            ...
            return ((ct.includes('csv') || ct.includes('pdf') || ct.includes('excel') || ct.includes('octet-stream')) ||
        ...
        const printCalls = await page.evaluate(() => window.__print_calls || 0);
        if (printCalls > 0) {
            return {
                route,
                selector,
 ...
 */
async function scanRoutes(page, routes) {
    const controls = [];
        ...
        try {
            ...
            for (let i = 0; i < allButtons.length; i++) {
                ...
                try {
                    ...
                    if (text.match(/export|download|print|csv|pdf|receipt|closeout|generate|report/i) ||
...
test_1.test.beforeEach(async ({ page }) => {
    await page.addInitScript(() => {
        ...
        window.print = function () {
            window.__print_calls = (window.__print_calls || 0) + 1;
            console.log(`[window.print] Called, count: ${window.__print_calls}`);
        };
    });
});
// SUBSET tests (for quick validation)
for (const roleConfig of SUBSET_ROLES) {
    (0, test_1.test)(`SUBSET: ${roleConfig.org}/${roleConfig.role}`, async ({ page }) => {
        console.log(`\n=== Testing ${roleConfig.org}/${roleConfig.role} ===`);
        // Login
        const loginResult = await (0, login_1.loginAsRole)(page, {
            org: roleConfig.org,
...
//# sourceMappingURL=print-export-job-contract.spec.js.map