...
async function getMenuItems(token, orgId) {
...
async function getBranches(token, orgId) {
...
async function getCategories(token, orgId) {
...
function addResult(check, passed, expected, actual) {
    results.push({ check, passed, expected, actual });
    const icon = passed ? '✅' : '❌';
    console.log(`${icon} ${check}: ${actual} (expected ${expected})`);
}
async function verifyOrg(name, email, password, expectedItems, expectedBranches) {
    console.log(`\n📦 Verifying ${name}...`);
    try {
        // Test login
        ...
        addResult(`${name} verification`, false, 'success', error.message);
    }
}
async function main() {
    ...
    // Verify Tapas
    await verifyOrg('Tapas Bar & Restaurant', 'owner@tapas.demo.local', 'Demo#123', 80, // Expected min menu items
    1 // Expected min branches
    );
    // Verify Cafesserie
    await verifyOrg('Cafesserie', 'owner@cafesserie.demo.local', 'Demo#123', 50, // Expected min menu items per branch
...
//# sourceMappingURL=verify-demo-data.js.map